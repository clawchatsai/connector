...
// env vars (MEMORY_PROVIDER, QDRANT_HOST, etc.) are read by plugin host (src/index.ts)
// and passed in via envOverrides to keep process.env out of the server bundle.
export function discoverMemoryConfig(envOverrides = {}) {
  ...
  if (envOverrides.qdrantUrl && !envOverrides.host) {
    try { const u = new URL(envOverrides.qdrantUrl); cfg.host = u.hostname; if (u.port) cfg.port = parseInt(u.port, 10); } catch { /* ignore */ }
  }
